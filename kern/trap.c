...
void Placment (struct Env*curenv, uint32 fault_va)
{
	...
	// return value no-mem then panic else continue
	if(ret1!=E_NO_MEM)
	{
		int ret2 = map_frame(curenv->env_page_directory ,ptr_frame_info  ,(void*)fault_va,PERM_PRESENT | PERM_USER | PERM_WRITEABLE);
		if(ret2!=E_NO_MEM)
		{
			// read the faulted page from page file to mem
			// connect frame (pa) with env
			int ret3 = pf_read_env_page(curenv,(void *)fault_va);
			// if value don't exist in env's page file then check if it's stack page
			...
			{
			 ret4 = pf_add_empty_env_page(curenv, fault_va, 1);

			    if (ret4 != E_NO_PAGE_FILE_SPACE)
				marker = 1;

			}
			 else //Not a Stack Page
			 {
			  panic("Not allowed memory u twat!");
			 }
	 ...
	 //Reflecting Changes in WorkingSet
	 if (marker == 1)
	 {
		int index = curenv->page_last_WS_index;
		for(int i = 0; i < curenv->page_WS_max_size; i++)
		{
			uint32 pageVirtual = curenv->ptr_pageWorkingSet[index].virtual_address;
			if(curenv->ptr_pageWorkingSet[index].empty || !(pt_get_page_permissions(curenv, pageVirtual) & PERM_PRESENT))
			{
				//found an empty slot in working set or found a page with present 0 , we add fault virtual address to Working set
...
int vaIndex ;
uint32 findingVVA(struct Env * curenv)
{
	...
	while(marker2 == 0)
	{
		if(marker == 1)
		{
			for(int i = 0; i < curenv->page_WS_max_size; i++)
			{
			  uint32 PageVirtual = curenv->ptr_pageWorkingSet[pos].virtual_address;
			  uint32 PagePerms = pt_get_page_permissions(curenv, PageVirtual);
			  int cont = !(PagePerms & PERM_USED) && !(PagePerms & PERM_MODIFIED);
			  if(cont)
			   {
				  //victim found
				  VVA = PageVirtual;
				  curenv->page_last_WS_index = pos;
				  vaIndex = pos;
				  marker2 = 1;
				  marker = 1;
				  break;
			  }
			  pos = (pos + 1)%curenv->page_WS_max_size;

			}
			//if no victim found go to 2nd Try(normal clock)
				marker3 = 1;
				marker = 0;
				continue;
		}
		...
		if(marker3 == 1)
		{
			for(int i = 0; i < curenv->page_WS_max_size; i++)
			{
					...
					if(cont)
				{
					...
					pt_set_page_permissions(curenv, PageVirtual, 0, PERM_USED);
					pos = (pos + 1)%curenv->page_WS_max_size;
				}
			    //if no victim found go again to Try 1
				marker = 1;
				marker3 = 0;
			}
		}
...
{//TODO: [PROJECT 2022 - [6] PAGE FAULT HANDLER]
	...
	//if Working set not full add the fault virtual to Working set
	if(env_page_ws_get_size(curenv) < curenv->page_WS_max_size)
	{
		Placment(curenv, fault_va);
	}
	//if Working set is full apply the Modified Clock Algorithm to replace fault virtual with the Victim
	else
	{
	int WS_M_size = curenv->page_WS_max_size ;
	int sizeOfWs = env_page_ws_get_size(curenv);
	uint32 PageVirtual =curenv->ptr_pageWorkingSet[curenv->page_last_WS_index].virtual_address;

	if (isPageReplacmentAlgorithmModifiedCLOCK())
	{
		uint32 * ptr_table = NULL ;
		uint32 VVA = findingVVA(curenv);
		struct Frame_Info * frame_info_ptr = get_frame_info(curenv->env_page_directory,(void *)VVA, &ptr_table);
		//check if victim is modified
		if ((pt_get_page_permissions(curenv, VVA) & PERM_MODIFIED))
		{
		 int ret = pf_update_env_page( curenv , (void *)VVA , frame_info_ptr);
		 if(ret==0)
		   {
			 unmap_frame(curenv->env_page_directory, (void *)VVA );
			 env_page_ws_clear_entry(curenv, vaIndex);
			Placment(curenv, fault_va);
		   }
