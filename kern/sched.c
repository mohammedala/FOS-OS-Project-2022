...
void sched_init_MLFQ(uint8 numOfLevels, uint8 *quantumOfEachLevel)
{
	...
	quantums = kmalloc(sizeof((uint8)*quantumOfEachLevel));
...
{
	if (curenv !=NULL){
		enqueue(&env_ready_queues[counter], curenv);
	}
	int i = 0 ;
	int marker = 0;
	struct Env* selected_env;
	do{
		marker = 0;
		if (queue_size(&env_ready_queues[i]) > 0){
			kclock_set_quantum(quantums[i]);
			selected_env = dequeue(&env_ready_queues[i]);
			marker = 1;
		}

		if (marker == 1){
			if (num_of_ready_queues -1 == i)
				counter = num_of_ready_queues - 1;
			else
				counter = i + 1;
			return selected_env;
