...
void * create_page_table(uint32 *ptr_page_directory, const uint32 virtual_address)
{
	...
	uint32* Page_table = (void *)kmalloc(PAGE_SIZE);
	memset((uint32*)Page_table,0,PAGE_SIZE);
	uint32* ptr_PA_page_table=(uint32*)kheap_physical_address((uint32)Page_table);
	ptr_page_directory[PDX(virtual_address)]=CONSTRUCT_ENTRY((uint32 )ptr_PA_page_table ,PERM_PRESENT | PERM_USER | PERM_WRITEABLE);
	tlbflush();
...
void allocateMem(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	int numDeFauilles = size / PAGE_SIZE;

	for (int i = 0 ; i<numDeFauilles; i++){
...
void freeMem(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	//TODO: [PROJECT 2022 - [12] User Heap] freeMem() [Kernel Side]
		...
		//This function should:
		size =  ROUNDUP(size, PAGE_SIZE);
		int numereDeFeuille = size /PAGE_SIZE;
		for(uint32 i = 0, VVA = virtual_address  ; i < numereDeFeuille; ++i , VVA = VVA + PAGE_SIZE)
		{
			pf_remove_env_page(e,VVA);
		}

		struct Frame_Info* frame_zeft = NULL;
		uint32* tableDeFueillle;

		for(int i = 0; i < e-> page_WS_max_size; i++)
		{
			uint32 VVAws = env_page_ws_get_virtual_address(e,i);
			int marker = 0;
			uint32 total = (virtual_address + ROUNDUP(size, PAGE_SIZE));
			if((VVAws < total) && (VVAws>=virtual_address))
			{
				marker = 1;
			}
			if (marker == 1){
				unmap_frame(e->env_page_directory,(void*)VVAws);
				env_page_ws_clear_entry(e,i);
			}
		}


		for(uint32 i=0, VVA = virtual_address; i<numereDeFeuille; VVA+=PAGE_SIZE, i++)
		{
			tableDeFueillle = NULL;
			int marker=0;
			get_page_table(e-> env_page_directory,(void*)VVA, &tableDeFueillle);
			if(tableDeFueillle != NULL)
			{
				int count = 0;
				int marker2 = 0;
				do {
					marker2 = 0;
					if(tableDeFueillle[count] != 0)
					{
						marker2 = 1;
					}
					if (marker2 == 1){
				...
				if(marker != 1)
				{
					pd_clear_page_dir_entry(e,(uint32)VVA);
					kfree((void*)tableDeFueillle);
				}
			}
		}
